...
get_input_config () {
    ...
    while (!meet_exit_criterion ()) {
        ...
        if ((ptr = strstr (app_data.line, INPUT_NUM)) != NULL) {
        ...
        } else if ((ptr = strstr (app_data.line, ENABLE_ROI)) != NULL) {
            ptr = get_value (ptr, ENABLE_ROI);
            if (ptr) {
                if (!strncasecmp (ptr, TRUE_VAL, strlen (TRUE_VAL))) {
                    app_data.ipParam[cnt-1].enable_roi = TRUE;
                } else if (!strncasecmp (ptr, FALSE_VAL, strlen (FALSE_VAL ))) {
                    app_data.ipParam[cnt-1].enable_roi = FALSE;
                } else {
        ...
        } else if ((ptr = strstr (app_data.line, ENABLE_LLP2)) != NULL) {
            ptr = get_value (ptr, ENABLE_LLP2);
            if (ptr) {
                if (!strncasecmp (ptr, TRUE_VAL, strlen (TRUE_VAL))) {
                    app_data.ipParam[cnt-1].enable_llp2 = TRUE;
                } else if (!strncasecmp (ptr, FALSE_VAL, strlen (FALSE_VAL ))) {
                    app_data.ipParam[cnt-1].enable_llp2 = FALSE;
                } else {
            ...
            if (ptr) {
                if (!strncasecmp (ptr, HDMI_2_INPUT, strlen (HDMI_2_INPUT)) || !strncasecmp (ptr, HDMI_RX_2_INPUT, strlen(HDMI_RX_2_INPUT))) {
                    ...
                    app_data.ipParam[cnt-1].device_type = CSI_2;
                    app_data.ipParam[cnt-1].src_type = LIVE_SRC;
                } else if (!strncasecmp (ptr, CSI_3_INPUT, strlen (CSI_3_INPUT))) {
                    app_data.ipParam[cnt-1].device_type = CSI_3;
                    app_data.ipParam[cnt-1].src_type = LIVE_SRC;
                } else if (!strncasecmp (ptr, CSI_4_INPUT, strlen (CSI_4_INPUT))) {
                    app_data.ipParam[cnt-1].device_type = CSI_4;
                    app_data.ipParam[cnt-1].src_type = LIVE_SRC;
        ...
        } else if ((ptr = strstr (app_data.line, RELATIVE_QP)) != NULL) {
            ptr = get_value (ptr, RELATIVE_QP);
            if (ptr) {
                app_data.ipParam[cnt-1].relative_qp = atoi (ptr);
            } else {
...
get_encoder_config () {
    ...
    while (!meet_exit_criterion ()) {
        ...
        if ((ptr = strstr (app_data.line, ENCODER_NUM)) != NULL) {
            ...
            if (ptr) {
                if (!strncasecmp (ptr, UNIFORM_QP, strlen (UNIFORM_QP))) {
                    ...
                    app_data.encParam[cnt-1].qp_mode = ROI;
                } else {
        ...
        } else if ((ptr = strstr (app_data.line, HLG_SDR_COMPATIBLE)) != NULL) {
            ptr = get_value (ptr, HLG_SDR_COMPATIBLE);
            if (ptr) {
                if (!strncasecmp (ptr, TRUE_VAL, strlen (TRUE_VAL))) {
                    app_data.encParam[cnt-1].hlg_sdr_compatible = TRUE;
                } else if (!strncasecmp (ptr, FALSE_VAL, strlen (FALSE_VAL))) {
                    app_data.encParam[cnt-1].hlg_sdr_compatible = FALSE;
                } else {
       ...
       } else if ((ptr = strstr (app_data.line, MAX_PICTURE_SIZE)) != NULL) {
           ptr = get_value (ptr, MAX_PICTURE_SIZE);
           if (ptr) {
               if (!strncasecmp (ptr, TRUE_VAL, strlen (TRUE_VAL))) {
                   app_data.encParam[cnt-1].max_picture_size = TRUE;
               } else if (!strncasecmp (ptr, FALSE_VAL, strlen (FALSE_VAL))) {
                   app_data.encParam[cnt-1].max_picture_size = FALSE;
               } else {
...
free_resources (void) {
    ...
    for (i =0; i< MAX_NUM_SOURCES; i++) {
	    ...
	    if (app_data.audParam[i].enable_audio) {
	        if(app_data.audParam[i].source_id != NULL) {
                g_free(app_data.audParam[i].source_id);
	        }
	        if (app_data.audParam[i].sink_id != NULL) {
                g_free(app_data.audParam[i].sink_id);
	        }
        }
    }
    if (app_data.cmnParam.bus_id) {
        g_free (app_data.cmnParam.bus_id);
    }
    if (app_data.file) {
        fclose (app_data.file);
        app_data.file = NULL;
...
reset_all_params () {
    ...
    for (i =0; i< MAX_NUM_SOURCES; i++) {
        ...
        app_data.encParam[i].hlg_sdr_compatible = FALSE;
        app_data.encParam[i].latency_mode = NORMAL_LATENCY;
        app_data.encParam[i].gdr_mode = GDR_MODE_DISABLED;
        if (app_data.encParam[i].enc_type == HEVC)
            app_data.encParam[i].entropy_mode = ENTROPY_MODE_CABAC;
        else
            app_data.encParam[i].entropy_mode = ENTROPY_MODE_CAVLC;
        app_data.encParam[i].max_picture_size = FALSE;
        ...
        app_data.ipParam[i].relative_qp = DEFAULT_RELATIVE_QP;
        app_data.ipParam[i].accelerator = FALSE;
        app_data.ipParam[i].enable_scd = FALSE;
        app_data.ipParam[i].enable_roi = FALSE;
        app_data.ipParam[i].enable_llp2 = FALSE;
...
print_pipeline_cmd () {
    ...
    for (cnt =0; cnt< app_data.cmnParam.num_src; cnt++) {
        ...
        if (!(app_data.ipParam[cnt].src_type == FILE_SRC || app_data.ipParam[cnt].src_type == STREAMING_SRC || app_data.ipParam[cnt].raw)) {
            ...
            g_print ("Qp Mode : %s\n", app_data.encParam[cnt].qp_mode == AUTO ? AUTO_QP : app_data.encParam[cnt].qp_mode == UNIFORM ? UNIFORM_QP : ROI_QP);
            ...
            g_print ("HLG_SDR_Compatible : %s\n", app_data.encParam[cnt].hlg_sdr_compatible == TRUE ? "True" : "False");
            g_print ("Latency Mode : %s\n", app_data.encParam[cnt].latency_mode == NORMAL_LATENCY ? NORMAL : SUB_FRAME);
            g_print ("GDR Mode : %s\n", app_data.encParam[cnt].gdr_mode == GDR_MODE_VERTICAL ? VERTICAL_GDR_MODE : app_data.encParam[cnt].gdr_mode == GDR_MODE_HORIZONTAL ? HORIZONTAL_GDR_MODE : DISABLED_GDR_MODE);
            g_print ("Entropy Mode : %s\n", app_data.encParam[cnt].entropy_mode == ENTROPY_MODE_CAVLC ? CAVLC_ENTROPY_MODE : app_data.encParam[cnt].entropy_mode == ENTROPY_MODE_CABAC ? CABAC_ENTROPY_MODE : DEFAULT_ENTROPY_MODE);
            g_print ("Max Picture Size : %s\n", app_data.encParam[cnt].max_picture_size == TRUE ? "True" : "False");
        }
        ...
        g_print ("Enable_roi flag : %s\n", app_data.ipParam[cnt].enable_roi == TRUE ? "True" : "False");
        g_print ("Enable_llp2 flag : %s\n", app_data.ipParam[cnt].enable_llp2 == TRUE ? "True" : "False");
